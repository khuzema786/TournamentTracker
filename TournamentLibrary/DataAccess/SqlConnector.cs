	...
	public class SqlConnector : IDataConnection
	{
		...
		public void CreatePrize(PrizeModel model)
		{
			using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(GlobalConfig.CnnString(db)))
			{
				...
				p.Add("@id", 0, dbType: DbType.Int32, direction: ParameterDirection.Output); // Output variable from SQL procedure
            ...
            foreach (List<MatchupModel> round in model.Rounds)
            {
                foreach (MatchupModel matchup in round)
                {
                    ...
                    foreach (MatchupEntryModel entry in matchup.Entries)
                    {
						...
						if(entry.ParentMatchup == null) // For first round there is no parent matchup
                        {
						...
						// For round 2 onwards, since no matchup has been played therefore no winners have been advanced so one of the team is present whereas another team is null
			...
			using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(GlobalConfig.CnnString(db)))
			{
                ...
                foreach (TournamentModel tm in output)
                {
                    ...
                    foreach (MatchupModel m in matchups)
                    {
						...
						// Populate each matchup entry (2 models)
